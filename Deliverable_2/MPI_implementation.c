...
/*
  1D distribution (row-wise cyclic):
   - Entry (i,j) goes to rank (i % size)
   - Stored as local row = i / size, col = j (global col)
*/
static void read_and_distribute_1D(
    ...
    char *buffer = (char*)malloc((size_t)read_size + 1);
    if (!buffer) {
        fprintf(stderr, "[Rank %d] ERROR: buffer malloc failed\n", rank);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    // Skip partial first line if not rank 0
    if (rank != 0) {
        while (p < q && *p != '\n') p++;
        if (p < q) p++;
    }
    // Truncate last partial line if not last rank
    if (rank != size - 1) {
        while (q > p && *(q - 1) != '\n') q--;
    }

    int cap = 4096;
    int *lr = (int*)malloc((size_t)cap * sizeof(int));
    int *lc = (int*)malloc((size_t)cap * sizeof(int));
    double *lv = (double*)malloc((size_t)cap * sizeof(double));
    if (!lr || !lc || !lv) {
        fprintf(stderr, "[Rank %d] ERROR: local COO malloc failed\n", rank);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    while (ptr < q && *ptr) {
        ...
        if (sscanf(ptr, "%d %d %lf", &r1, &c1, &v) == 3) {
            ...
            if ((i % size) == rank) {
                if (count == cap) {
                    cap *= 2;
                    lr = (int*)realloc(lr, (size_t)cap * sizeof(int));
                    lc = (int*)realloc(lc, (size_t)cap * sizeof(int));
                    lv = (double*)realloc(lv, (size_t)cap * sizeof(double));
                    if (!lr || !lc || !lv) {
                        fprintf(stderr, "[Rank %d] ERROR: realloc failed\n", rank);
                        MPI_Abort(MPI_COMM_WORLD, 1);
                    }
                }
                lr[count] = i / size; // local row index
                lc[count] = j;        // keep GLOBAL column index
                lv[count] = v;
                count++;
            }
        }
...
static void coo_to_csr(
    ...
    *vals    = (double*)malloc((size_t)local_nnz * sizeof(double));
    ...
    for (int r = 0; r < local_rows; r++) (*row_ptr)[r + 1] += (*row_ptr)[r];
...
int main(int argc, char **argv) {
    ...
    read_and_distribute_1D(argv[1], rank, size, &rows, &cols, &global_nnz,
    ...
    for (int r = 0; r < local_num_rows; r++) {
        ...
        for (int k = row_ptr[r]; k < row_ptr[r + 1]; k++) {
            int j = col_idx[k]; // GLOBAL col index
            acc += vals[k] * x[j];
        }
        y_local[r] = acc;
    }
    ...
    t0 = MPI_Wtime();
    MPI_Gatherv(y_local, sendcount, MPI_DOUBLE,
                y_gather, recvcounts, displs, MPI_DOUBLE,
                0, MPI_COMM_WORLD);
    t_comm += MPI_Wtime() - t0;
    ...
    if (rank == 0) {
        ...
        // Reorder to true row order (since gathered by rank blocks)
        ...
        /*
        printf("y_correct[0..9]: ");
        for (int i = 0; i < 10 && i < rows; i++) printf("%.3f ", y_correct[i]);
        printf("\n");
        */
