...
echo
echo "---------------------------------------------"
echo "WEAK SCALING 2D hybrid MPI+OpenMP #pragma omp parallel schedule(runtime)"
echo "---------------------------------------------"
for P in "${PROCS[@]}"; do
...
echo "---------------------------------------------"
echo "WEAK SCALING 1D hybrid MPI+OpenMP #pragma omp parallel schedule(runtime)"
echo "---------------------------------------------"
for P in "${PROCS[@]}"; do
    MTX="${MATRIX_DIR}/weak_ps${P}.mtx"
