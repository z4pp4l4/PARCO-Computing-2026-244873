...
typedef struct {
    int *Arow;      // row pointer (size nrows+1)
    int *Acol;      // column indices (size nnz)
    double *Aval;   // nonzero values (size nnz)
    int nrows;
    int ncols;
    int nnz;
} CSR;
...
/**
 * Reads a Matrix Market (.mtx) file in coordinate format
 * and converts it into CSR (Compressed Sparse Row) format.
 */
/*
 CSR *read_matrix_market_to_CSR(const char *filename) {
    ...
    for (int i = 0; i < nnz; i++) {
        if (fscanf(f, "%d %d %lf", &coo_row[i], &coo_col[i], &coo_val[i]) != 3) {
            fprintf(stderr, "Error reading matrix data at line %d.\n", i + 1);
            exit(EXIT_FAILURE);
        }
        coo_row[i]--;  // convert 1-based to 0-based
        coo_col[i]--;
    }
    fclose(f);
    ...
    // Fill CSR arrays
    int *counter = calloc(rows, sizeof(int));
    for (int i = 0; i < nnz; i++) {
        int r = coo_row[i];
        int dest = csr->Arow[r] + counter[r];
...
void print_CSR(CSR *csr) {
    ...
    printf("\nAval: ");
    for (int i = 0; i < csr->nnz; i++)
        printf("%.2f ", csr->Aval[i]);
    printf("\n");
}

int main() {
    char filename[256];
    printf("Enter path of Matrix Market file (.mtx): ");
    scanf("%255s", filename);
...
CSR *read_matrix_market_to_CSR(const char *filename) {
    ...
    for (int i = 0; i < nnz; i++) {
        ...
        coo_row[i]--;  // convert 1-based → 0-based
        coo_col[i]--;
    }
    fclose(f);
...
void export_CSR_to_header(CSR *csr, const char *basename) {
    ...
    fprintf(out, "#ifndef %s_CSR_H\n", basename);
    fprintf(out, "#define %s_CSR_H\n\n", basename);
    ...
    for (int i = 0; i < csr->nnz; i++)
        fprintf(out, "%.6f%s", csr->Aval[i], (i == csr->nnz - 1) ? "" : ", ");
    fprintf(out, "};\n\n");

    fprintf(out, "#endif // %s_CSR_H\n", basename);
    fclose(out);
...
int main() {
    ...
    char *base = strtok(temp, ".");
    if (!base) base = filename;

    CSR *csr = read_matrix_market_to_CSR(fullpath);
    export_CSR_to_header(csr, base);
...
/**
 * Reads a Matrix Market (.mtx) file and converts it to CSR format.
 * Supports both 'general' and 'symmetric' matrices.
 */
CSR *read_matrix_market_to_CSR(const char *filename) {
    FILE *f = fopen(filename, "r");
    if (!f) {
...
/**
 * Export CSR matrix as a .h header file for inclusion in other C programs.
 */
void export_CSR_to_header(CSR *csr, const char *basename) {
    char outpath[512];
    snprintf(outpath, sizeof(outpath), "%s_csr.h", basename);
